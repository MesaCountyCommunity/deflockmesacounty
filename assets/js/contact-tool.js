// The Mesa County contact tool, embedded as the Take Action section of the
// campaign page rather than owning the whole document.
//
// Two things differ from the standalone version it was ported from:
//
//  1. Every container id is `ct-`-prefixed. The campaign page has its own
//     <nav>, <footer>, and tab-like controls (public comment scripts), and
//     the unprefixed ids the standalone tool used (#tabs, #panel, #footer,
//     #output) would have collided with them.
//  2. Locale selection writes `#contact-<id>` via replaceState instead of
//     assigning location.hash. The page's own nav owns hashes like `#act`
//     and `#findings`; a bare `#fruita` would have fought them, and
//     assigning location.hash on every tab click would bury the section
//     anchors under a pile of history entries.

import { EMAIL_RE } from '../../scraper/validate.js';

const HASH_PREFIX = 'contact-';
...
// Reads a locale id out of the URL only if the hash is one of ours, so a
// page anchor (#act, #signup) never gets mistaken for a locale.
function localeFromHash() {
  const hash = location.hash.replace('#', '');
  if (!hash.startsWith(HASH_PREFIX)) return null;
  const id = hash.slice(HASH_PREFIX.length);
  return state.locales.some(l => l.id === id) ? id : null;
}
...
async function init() {
  ...
  renderTabs();
  activate(localeFromHash() ?? state.locales[0].id);
  addEventListener('hashchange', () => {
    const id = localeFromHash();
    if (id && id !== state.active) activate(id);
  });
}

function renderTabs() {
  const nav = document.getElementById('ct-tabs');
  nav.innerHTML = '';
...
// roving tabindex/focus state is synchronous) and records the choice in the
// URL so the tab is linkable. replaceState rather than assigning
// location.hash — see the note at the top of this file.
...
function onTabKeydown(e) {
  ...
  selectTab(newId);
  document.getElementById(`ct-tab-${newId}`).focus();
}
...
function activate(id) {
  ...
  document.getElementById('ct-panel').setAttribute('aria-labelledby', `ct-tab-${id}`);
...
function renderLocale(locale) {
  ...
  for (const body of locale.bodies) {
    ...
    // h3, not h2: the campaign page's section owns the h2 above this.
...
function updateActionBar() {
  ...
  go.addEventListener('click', () => showOutput());
...
function showOutput() {
  ...
  subject.id = 'ct-subject'; subject.placeholder = 'Subject';
  const message = document.createElement('textarea');
  message.id = 'ct-message'; message.rows = 12; message.placeholder = 'Your message';

  const salName = document.createElement('input');
  salName.id = 'ct-sal-name';
  salName.placeholder = 'Optional';
  let salDistrict = null;
  if (locale.districts) {
    salDistrict = document.createElement('select');
    salDistrict.id = 'ct-sal-district';
  ...
  // field. Rebuilding subject+message wholesale on every keystroke would
  // discard anything the resident had typed — and clobber the native undo
  // stack, since it's a programmatic .value write.
  let lastSalutation = '';
...
function renderUpdated(locale) {
  const { contacts } = state.dataById.get(locale.id);
  const el = document.getElementById('ct-updated');
...
init().catch(err => {
  ...
  message.className = 'ct-error';
  message.textContent = `Failed to load contact data: ${err.message}`;
  ...
  const links = document.getElementById('ct-official-links');
